...
func (m *Salesman) registerDeal(planID string, deal *sonm.Deal) error {
	...
	if plan.DealID.Cmp(deal.GetId()) == 0 {
		return nil
	}
	if !plan.DealID.IsZero() {
...
func (m *Salesman) placeOrder(ctx context.Context, plan *sonm.AskPlan) (*sonm.Order, error) {
	...
	if err := m.registerOrder(plan.ID, ordOrErr.Order); err != nil {
		return nil, err
	}
	m.log.Infof("placed order %s on blockchain", ordOrErr.Order.GetId().Unwrap().String())
